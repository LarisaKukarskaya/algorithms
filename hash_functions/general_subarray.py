...
def is_not_collision(a: List[int], b: List[int], n: int, a_indices: List[int], b_index: int) -> bool:
    return any(
        all(a[a_index + i] == b[b_index + i]) for i in range(n)
    for a_index in a_indices
    )

def len_of_subarray(a: List[int], b: List[int], n: int) -> bool:
    q_big = pow(q, n-1, m)
...
def longest_len_of_subarray(a: List[int], b: List[int]) -> int:
    if len(a) > len(b):
        a, b = b, a
...
if __name__ == '__main__':
    ...
    print(longest_len_of_subarray(a, b))    
